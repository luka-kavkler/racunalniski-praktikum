...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \(k_1n\) označimo število
kršitev omejitve \(n\), z \(u_n\) pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\[k_1 \cdot u_1 + k_2 \cdot u_2 + \dots + k_{n-1} \cdot k_{n-1} + k_n \cdot u_n\]
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \begin{enumerate}
      ...
      \item Za vse proste termine \(predavalnice_i\) izračunamo ceno vstavljanja
